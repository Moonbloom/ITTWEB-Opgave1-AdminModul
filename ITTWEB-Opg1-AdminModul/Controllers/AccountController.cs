...
namespace ITTWEB_Opg1_AdminModul.Controllers
{
    ...
    {
        private AppDbManager _appDbManager = new AppDbManager();
        ...
        {
            var users = _appDbManager.GetAllApplicationUsers();
        ...
        public ActionResult Delete(string id)
        {
            ...
            var user = _appDbManager.GetApplicationUser(id);
            if (user == null)
            {
                return HttpNotFound();
            }
        ...
        {
            _appDbManager.DeleteApplicationUser(id);
        ...
        public ActionResult Edit([Bind(Include = "Id,userName,roleName")] ApplicationUser user)
        {
            if (ModelState.IsValid)
            {
                var realRoleName = _appDbManager.GetRealRoleName(user.RoleName); //Hm
        ...
        private void AddAccToRole(string userName, string roleName)
        {
            var user = _appDbManager.GetApplicationUserByName(userName);
            if (user != null)
            {
                UserManager.RemoveFromRoles(user.Id, "Admin", "User");
                UserManager.AddToRole(user.Id, roleName);
            }
        }

        private void RemoveRole(string userName, string roleName)
        {
